...
describe('PowerOf2Image', () => {
  describe('should load image from given url', () => {
    ...
    let promiseResult;
    before('after onComplete', (done) => {
      p2img.onComplete.then((img) => {
        promiseResult = img;
        done();
      });
    });

    it('onComplete promise should return the PowerOf2Image instance', () => assert.equal(p2img, promiseResult));
  ...
  describe('should load image from HTMLImageElement', () => {
    ...
    before('after onComplete', () => p2img.onComplete);
